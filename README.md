...
- [x] Explain what a token is used for.
      ...
      A token is a device that employs an encrypted key for which the encryption algorithm - the method of generating an encrypted password - is known to a network's authentication server. The token is assigned to a user by linking it's serial number to the user's record, stored in the system database.

- [x] What steps can you take in your web apps to keep your data secure?

      You can implement protected routes that only allow access to those who have the authentication token provided by signing up and logging in. 

- [x] Describe how web servers work.

      A web server processes incoming network requests over HTTP and several other related protocals. the main functino of a web server is to store, process and deliver web pages to clients.

- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

      The CRUD acronym utilizes the GET, POST, PUT, and DELETE requests.
